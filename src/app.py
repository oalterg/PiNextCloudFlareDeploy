...
SETUP_STARTED_MARKER = f"{REPO_DIR}/.setup_started"
ENV_FILE = f"{REPO_DIR}/.env"
ENV_TEMPLATE = f"{REPO_DIR}/config/.env.template"
...
def start_setup():
    ...
    # Ensure we start from the robust template
    if not os.path.exists(ENV_FILE):
        if os.path.exists(ENV_TEMPLATE):
            shutil.copyfile(ENV_TEMPLATE, ENV_FILE)
        else:
            with open(ENV_FILE, "w") as f:
                f.write("# Auto-generated by Appliance Manager\n")
